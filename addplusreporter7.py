...
class SpellingCorrector:
    def __init__(self):
        ...
        # [수정] 조사가 아닌데 조사처럼 보이는 단어들 (교정 제외)
        self.exceptions = {
            # 1. 명사 (끝글자가 조사와 같지만 단어인 경우)
            '증가', '추가', '결과', '효과', '초과', '교과', '부과', '사과', '투과',
            '평가', '원가', '정가', '단가', '시가', # [추가] 평가 및 가격 관련 단어
            '사이', '차이', '나이', '아이', '오이', '놀이',
            '경로', '진로', '선로', '항로',
            # 2. 동사/형용사 활용형
    ...
    def run(self, text):
        ...
        for i, part in enumerate(parts):
            ...
            def josa_replacer(match):
                full_word = match.group(0) # 예: "평가", "없는"
                
                # [중요] 예외 목록에 있는 단어는 교정하지 않음
...
st.markdown("""
**1. 수식 조사 호응:** LaTeX 수식 뒤의 조사를 교정합니다.  
**2. 한글 맞춤법:** '평가', '증가' 등 조사처럼 끝나는 단어를 오탐지하지 않도록 보호합니다.
""")
