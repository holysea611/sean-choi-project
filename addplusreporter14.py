...
class JosaCorrector:
    def __init__(self):
        ...
        # ★ 수정: '이'가 지시어로 쓰이는 빈출 패턴 추가 (이 점, 이 값, 이 경우 등)
    ...
    def _init_batchim_dict(self):
        d = {
            ...
            'L': True, 'M': True, 'N': True, 'R': True, # M(엠), N(엔), L(엘), R(알) -> 받침 있음
    ...
    def _init_unit_batchim_dict(self):
        return {
            ...
            'N': True,  # 뉴턴(N) -> 받침 있음
            'M': True   # ★ 수정: 점 M(\mathrm{M}) 등도 '엠'으로 읽으므로 받침 있음으로 처리
        }
    ...
    def get_correct_p(self, target, original_p):
        # 보호 단어 체크 (이 점, 이 값 등 포함)
        for word in self.protected_words:
            if original_p.startswith(word): return original_p
            # 띄어쓰기가 포함된 보호 단어 처리 (예: '이 점')
            # original_p는 공백을 포함하지 않고 조사 부분만 넘어오므로,
            # 여기서 '이'만 넘어왔을 때 뒤에 ' 점'이 있는지 알 수 없음.
            # 따라서 이 로직은 replacer 함수 내에서 처리하는 것이 더 정확함.
            # 여기서는 단일 단어 보호만 수행.
        ...
        if target.startswith("UNIT:"):
            ...
            # ★ 수정: unit_batchim_dict에 M이 있으면 True 반환
    ...
    def run(self, raw_input):
        ...
        def replacer(match):
            ...
            remaining_particle = particle[p_start:] # '이 점은' -> '이 점은'
            
            # ★ 수정: 지시어(이, 그, 저) + 공백 + 명사 패턴 보호
            # particle 변수는 수식 뒤의 텍스트 전체(공백 포함)를 담고 있음.
            # 예: " 이 점은" (앞에 공백 있음) -> original_p="이"
            # 예: "이 점은" (앞에 공백 없음) -> original_p="이"
            
            # 1. protected_words 리스트 확인 (이 점, 이 값 등)
            for word in self.protected_words:
                # remaining_particle이 '이 점' 등으로 시작하는지 확인
    ...
    def run(self, text):
        ...
        for i, part in enumerate(parts):
            ...
            def josa_replacer(match):
                ...
                # ★ 수정: 지시어 '이', '그', '저'가 명사 자리에 오면 교정 제외
