...
class SpellingCorrector:
    ...
    def has_batchim(self, char):
        """한글 글자의 받침 유무 확인 (True: 받침 있음, False: 없음)"""
        if '가' <= char <= '힣':
    ...
    def correct_korean_josa(self, text):
        """
        한글 단어 뒤의 조사 호응 검사 (예: 사람는 -> 사람은)
        """
        # 정규식: [한글한글자] + [조사] + [한글이아닌것(공백,기호) 혹은 문장끝]
        # 주의: '마을' 같은 단어가 '마' + '을'(조사)로 오인되지 않게 뒤에 한글이 오지 않아야 함
        pattern = r'([가-힣])(은|는|이|가|을|를|과|와|으로|로)(?![가-힣])'
        
        def josa_replacer(match):
            noun_char = match.group(1) # 앞 글자 (예: '람')
            josa = match.group(2)      # 조사 (예: '는')
            ...
            for bat_o, bat_x in self.korean_particle_pairs:
                if josa == bat_o or josa == bat_x:
                    if bat_o == '으로': # (으로/로) 특수 처리
                        # ㄹ받침이거나 받침이 없으면 '로', 그 외 받침은 '으로'
                        if not has_bat or is_rieul:
                            correct_josa = '로'
                        else:
                            correct_josa = '으로'
                    else: # 일반 조사 (은/는, 이/가 등)
...
**2. 한글 맞춤법:** 수학 용어(최댓값 등) 및 **한글 단어 뒤의 조사(사람는 -> 사람은)**를 교정합니다.
""")
...
with col1:
    ...
    input_val = st.text_area("텍스트를 입력하세요:", height=600, 
                             placeholder="예: $Q(n)$이라 하고, 사과을 먹었다. 최대값은 얼마인가?")
...
with col2:
    ...
    if input_val:
        ...
        # --- 리포트 출력 ---
        tab1, tab2 = st.tabs(["🔍 수식 조사 검수", "📝 한글/맞춤법 검수"])
        ...
        with tab2:
            if spell_logs:
                st.warning(f"한글/맞춤법 오류: {len(spell_logs)}건")
