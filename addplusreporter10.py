...
class JosaCorrector:
    ...
    def find_target(self, formula_str):
        ...
        # 5. 텍스트만 추출 (명령어 제거)
        # \ln, \log, \sin 등 명령어 제거 후 남은 텍스트 확인
    ...
    def run(self, raw_input):
        ...
        def replacer(match):
            # 정규식 그룹: (prefix, space, delimiter, formula, space, particle)
            pre, s1, delim, formula, s2, particle = match.groups()
            
            # [중요] JSON 등에서 이스케이프된 역슬래시(\\)를 정규화(\)
            ...
            if not p_match:
                if '.' in particle:
                    ...
                    return f"{pre}{s1}{delim}{formula}{delim}{s2}{new_particle}"
            ...
            return f"{pre}{s1}{delim}{formula}{delim}{s2}{particle[:p_start]}{correct_p}{particle[p_match.end():]}"

        # [수정] $$...$$ 또는 $...$ 모두 매칭되도록 정규식 개선
        # (\$+): $가 1개 이상 (delimiter)
        # ([^\$]+): $가 아닌 내용 (formula)
        # \3: 앞서 매칭된 delimiter와 동일한 개수의 $
        pattern = r'([^$]*?)(\s*)(\$+)([^\$]+)\3(\s*)([\s,]*[가-힣\s\.\?\!]+)'
