...
class JosaCorrector:
    ...
    def find_target(self, formula_str):
        ...
        # [수정 1] 끝부분에 남은 역슬래시나 공백 제거 (LaTeX 추출 오류 방지)
        ...
        if "^" in final_term:
            ...
            # 제곱 처리 시에도 mathrm이 있으면 단위 확인 (단, 제곱 로직 내부에서 처리)
        ...
        # [수정 2] 괄호로 끝나는 경우를 mathrm 확인보다 '먼저' 체크
        # 예: \mathrm{N}(0, 1) -> 1로 인식해야 함 (N으로 인식하면 안 됨)
        if final_term.endswith(')'):
             m = re.search(r'([가-힣a-zA-Z0-9])\)+$', final_term)
             if m: return m.group(1)

        # [수정 3] mathrm은 괄호 검사 후, 명시적인 단위나 함수명이 단독으로 쓰였을 때 처리
